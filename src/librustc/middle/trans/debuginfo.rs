...
fn describe_enum_variant(cx: &CrateContext,
    ...
    let variant_definition_span = if variant_info.id.crate == ast::LOCAL_CRATE {
        match cx.tcx.map.find(variant_info.id.node) {
            Some(ast_map::NodeVariant(ref variant)) => variant.span,
            ref node => {
